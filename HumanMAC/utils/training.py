...
class Trainer:
    ...
    def run_train_step(self,mode="one shot"): #"iterative"-> all the way to recover original size; one shot-> use difference

        for traj_np in tqdm(self.generator_train):
            with torch.no_grad():
                ...
                input_frames = traj[:,:self.cfg.t_his,:]
            ...
            if mode== "iterative":
                ...
                predicted_noise,recon = self.model(x_t, t, mod=traj_dct_mod,stage="train")
                result_list=[]
                for idx in range(t.shape[0]):
                    for sample in self.diffusion.recon(t[idx], x_t[idx], recon[idx]):
                        final=sample

                    result_list.append(final)

                recon=torch.stack(result_list,dim=0)
                recon = torch.matmul(self.cfg.idct_m_all[:,:self.cfg.n_pre], recon)[:,:self.cfg.t_his,:]
                
                loss = self.criterion(predicted_noise, noise) + 0.1*self.criterion(recon, input_frames)
            elif mode=="one shot":
                x_t_minus_1, prev_noise = self.diffusion.noise_motion(traj_dct, t-torch.ones_like(t))
                x_t, noise = self.diffusion.noise_motion_incremental(x_t_minus_1,t, prev_noise)
                predicted_noise,recon = self.model(x_t, t, mod=traj_dct_mod,stage="train")

                gt_frames = torch.matmul(self.cfg.idct_m_all[:,:self.cfg.n_pre],x_t_minus_1)
                recon = self.diffusion.compute_prev_t(t,x_t,recon)
                loss = self.criterion(predicted_noise, noise) + 0.1*self.criterion(recon, gt_frames)
