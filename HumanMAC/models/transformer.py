...
class MotionTransformer(nn.Module):
    def __init__(self,
        ...
        self.stage_decoder = zero_module(nn.Linear(self.latent_dim, self.input_feats))

    def forward(self, x, timesteps, mod=None,stage="valid"):
        """
        x: B, T, D
        """
        ...
        recon = self.stage_decoder(feature).view(B, T, -1).contiguous()
        if stage=="train":
            return output,recon
        else:
            return output #B,T,48
