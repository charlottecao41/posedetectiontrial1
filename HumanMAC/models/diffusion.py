...
class Diffusion:
    ...
    def noise_motion_incremental(self, xt_1, t, noise):
        '''
        input xt_1,t,noise between x0 xt-1
        output xt, true noise between x0 xt
        '''
        alpha_t = self.alpha_hat[t]/self.alpha_hat[t-torch.ones_like(t)]
        alpha_hat_t = self.alpha_hat[t]
        sqrt_alpha = torch.sqrt(alpha_t)[:, None, None]
        sqrt_one_minus_alpha = torch.sqrt(1 - alpha_t)[:, None, None]
        Ɛ = torch.randn_like(xt_1)
        
        alpha_hat_t_1 = self.alpha_hat[t-torch.ones_like(t)]
        sqrt_one_minus_alpha_hat_minus_1 = torch.sqrt(1-alpha_hat_t_1)[:, None, None]
        sqrt_one_minus_alpha_hat = torch.sqrt(1-alpha_hat_t)[:, None, None]

        true_noise = (sqrt_one_minus_alpha * Ɛ + sqrt_one_minus_alpha_hat_minus_1*noise*sqrt_alpha)/sqrt_one_minus_alpha_hat
        return sqrt_alpha * xt_1 + sqrt_one_minus_alpha * Ɛ, true_noise

    def sample_timesteps(self, n):
        #NOISE STEPS = 1000
        #NOTE:  MINUS ONE SO IT WILL WORK
    ...
    def compute_prev_t(self,t,x_t,noise):

        alpha_hat = self.alpha_hat[t][:, None, None]
        alpha_hat_prev = self.alpha_hat[t-torch.ones_like(t)][:, None, None]

        predicted_x0 = (x_t - torch.sqrt((1. - alpha_hat)) * noise) / torch.sqrt(alpha_hat)
        pred_dir_xt = torch.sqrt(1 - alpha_hat_prev) * noise
        x_prev = torch.sqrt(alpha_hat_prev) * predicted_x0 + pred_dir_xt

        x_prev = torch.matmul(self.idct[:, :self.n_pre],x_prev)

        return x_prev

    

    def recon(self, t, x_t, noise):
        ...
        for i in reversed(range(0, t)):
            ...
            # alpha_hat_prev = self.alpha_hat[i-torch.ones_like(t)*mask]
            alpha_hat_prev = self.alpha_hat[i-1]

            predicted_noise = noise

            predicted_x0 = (x - torch.sqrt((1. - alpha_hat)) * predicted_noise) / torch.sqrt(alpha_hat)
            # predicted_x0 = (x*mask - torch.sqrt((1. - alpha_hat*mask)) * predicted_noise*mask) / torch.sqrt(alpha_hat*mask)
            pred_dir_xt = torch.sqrt(1 - alpha_hat_prev) * predicted_noise

            #pred_dir_xt = torch.sqrt(1 - alpha_hat_prev*mask) * predicted_noise
            x_prev = torch.sqrt(alpha_hat_prev) * predicted_x0 + pred_dir_xt
            #x_prev = torch.sqrt(alpha_hat_prev) * predicted_x0 + pred_dir_xt
